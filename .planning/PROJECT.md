...
---
*Last updated: 2026-05-09 after v2.0 MVP milestone initialization*

## Current Milestone: v2.0 商业化 MVP

**Goal:** 搭建 Web-app 基础架构 + Extension 云端同步功能，实现商业化第一步。

**Target features:**

1. **Web-app 基础搭建（Phase 2）**
   - packages/web-app Next.js 全栈应用
   - Supabase 项目配置与 Database Schema
   - 基础同步 API（/api/sync/upload, /api/sync/download）
   - 落地页（首页、定价页）

2. **Extension 云端同步（Phase 3）**
   - packages/extension/src/sync/ 模块
   - Supabase Client + OAuth 登录弹窗
   - 同步 UI（按钮 + 进度提示）
   - 上传/下载同步逻辑

**Key context:**
- 参考：docs/superpowers/specs/2026-05-08-commercialization-architecture-design.md
- 约束：localhost:3000 开发，新建 Supabase 项目
- 暂不实施：Phase 4-5（Stripe、订阅、AI优化、团队协作）
